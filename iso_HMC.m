...
[chain,endpoint,accratio] = drawSamples(smp, Burnin=100, NumSamples=100);
save("samples_Nsteps10_Burn100.mat", "chain", "endpoint", "accratio");

function [nlp, grad] = log_post_obj(x, kD, obs, lnsx, lnsy, f, Nx, dr, T, priors_new)
    disp(compose(strjoin(repmat("%6.2f", length(x), 1), " "), x(:).'))
    if nargout < 2
